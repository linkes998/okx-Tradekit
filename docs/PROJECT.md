...
| `rh_live_trader.py`      | 535  | Desk signal\_callback → quote → build\_bundle 桥接 |
...
| **可回测**           | pool\_lake.py 支持历史 OHLCV 回测 Desk 表现      |
...
- [x] LiveTrader 桥接层 (signal\_callback → quote → build\_bundle)
